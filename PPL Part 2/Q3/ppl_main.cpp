...
void pair1() {
	...
	fclose(fp3);
	for(int i = 0; i < 10; i++) { // making couples accordint to the parameters given
		if(vecg[i].status == 0) {
			int max = 0,k=-1;
			for(int j = 0;j<20;j++) {
...
int main() {
	...
	for( int j = 0 ; j < t ; j++ ) {
		couple c = vecc[j] ;
		c.get_basket(&vecb,&vecg,vecgi); // making baskets for couples
		c.seth( vecb , vecg ); // setting couple's happines
		c.setco( vecb , vecg ) ; // setting couple's compatibility
		vecc[j] = c;
	}
	int k = 5 ;
	sort ( vecc.begin() , vecc.end() , cmp1 ) ;
	cout << "5 Most Happy Couples:\n" ;
	for ( int i = 0 ; i < min ( k, (int)vecc.size() ) ; i++ ) {
		cout << vecc[i].girln << " " << vecc[i].boyn << endl ;
	}
	sort ( vecc.begin() , vecc.end() , cmp2 ) ;
	cout << "\n5 Most Compatible Couples:\n" ;
	for ( int i = 0 ; i < min ( k, (int)vecc.size() ) ; i++ ) {
		cout << vecc[i].girln << " " << vecc[i].boyn << endl ;
	}
	return 0;
