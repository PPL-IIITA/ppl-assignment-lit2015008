...
class couple{
	public:
		...
		void get_basket(vector<boy> *vb,vector <girl> *vg, vector<gift> vgi) { // creating gift basket for each couple
		...
		void seth(vector<boy> vb , vector <girl> vg ) {
			...
			couplehap = boyhap + girlhap; // method for finding couple's happiness
		}
		void setco(vector<boy> vb,vector <girl> vg) {
			...
			comp += ( abs( vb[b].intelligence- vg[g].intelligence ) ) ; // method for finding couple's compatibilty
		}
};
#endif
